...
if !SOFTWARE_LOADED! equ 1 (
    ...
    reg delete "HKLM\MOUNTED_SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\DelegateFolders\{F5FB2C77-0E2F-4A16-A381-3E560C68BC83}" /f >nul 2>&1 && set /a "REG_SUCCESS+=1" || set /a "REG_FAILED+=1"
    ...
    if "%TARGET_WINDOWS_VER%"=="11" (
        echo     - Windows 11: Menonaktifkan Fitur AI, Copilot, dan Telemetri...
        ...
        echo     - Windows 11: Menonaktifkan Iklan, Widget, dan Konten Online...
...
if !DEFAULT_LOADED! equ 1 (
    ...
    :: Windows 11 specific tweaks
    if "%TARGET_WINDOWS_VER%"=="11" (
        echo     - Windows 11: Tweak Privacy dan Content Delivery...
        ...
        echo     - Windows 11: Tweak Explorer dan Search...
        ...
        echo     - Windows 11: Setup RunOnce untuk Context Menu dan Explorer Tweaks...
        ...
        echo     - Windows 11: Tweak Teams Auto-Start...
        reg add "HKLM\MOUNTED_DEFAULT\Software\Microsoft\Office\Teams" /v "HomeUserAutoStartAfterInstall" /t REG_DWORD /d 0 /f >nul 2>&1 && set /a "REG_SUCCESS+=1" || set /a "REG_FAILED+=1"
        reg add "HKLM\MOUNTED_DEFAULT\Software\Microsoft\Office\Teams" /v "PreventFirstLaunchAfterInstall" /t REG_DWORD /d 1 /f >nul 2>&1 && set /a "REG_SUCCESS+=1" || set /a "REG_FAILED+=1"
        reg add "HKLM\MOUNTED_DEFAULT\Software\Policies\Microsoft\Office\Teams" /v "PreventFirstLaunchAfterInstall" /t REG_DWORD /d 1 /f >nul 2>&1 && set /a "REG_SUCCESS+=1" || set /a "REG_FAILED+=1"
        reg add "HKLM\MOUNTED_DEFAULT\Software\Policies\Microsoft\Office\Teams" /v "AutoStart" /t REG_DWORD /d 0 /f >nul 2>&1 && set /a "REG_SUCCESS+=1" || set /a "REG_FAILED+=1"
    )
)
...
if !SYSTEM_LOADED! equ 1 (
    ...
    :: Windows 11 specific tweaks (Standard/Consumer only, not LTSC)
    if "%TARGET_WINDOWS_VER%"=="11" if not "%TARGET_WINDOWS_TYPE%"=="LTSC" (
        echo     - Windows 11: Bypass System Requirements...
